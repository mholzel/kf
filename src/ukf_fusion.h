...
private:
    ...
    /** A handy function for retrieving the filter of the specified type and throwing an error if it doesn't exist. */
    Measurement<T, x_size, y_size> *getMeasurementModel(const EventType &type) {
        auto measurement_model = measurement_models.find(type);
        // TODO This should be an assert, but to satisfy the project goals we comment it out
    ...
    static T defaultLambda(int size) {
        return 3 - size;
    }
...
public:
    ...
    UKFFusion(T &t,
              ...
              Dynamics<T, x_size, v_size> &dynamics,
              std::unordered_map<int, Measurement<T, x_size, y_size> *> &measurement_models)
            : t(t), x(x), P(P), dynamics(dynamics), measurement_models(measurement_models) {
    }

    ~UKFFusion() {};

    template<int size>
    static Eigen::Matrix<T, size, 2 * size + 1>
    generateSigmaPoints(Eigen::Matrix<T, size, 1> &x,
                        Eigen::Matrix<T, size, size> &P) {

        const T lambda = defaultLambda(size);
        Eigen::Matrix<T, size, size> sqrtP = P.llt().matrixL();
        sqrtP *= sqrt(lambda + size);
        Eigen::Matrix<T, size, 2 * size + 1>
                sigma_points = Eigen::Matrix<T, size, 2 * size + 1>::Zero();
    ...
    ukfMean(const Eigen::Matrix<T, size, cols> &x,
            const int n_aug = size + v_size) {
        const T lambda = defaultLambda(n_aug);
        const T mean_weight = lambda / (lambda + n_aug);
        const T other_weight = 1 / (2 * (lambda + n_aug));
        return other_weight * x.rowwise().sum() + (mean_weight - other_weight) * x.col(0);
    }

    template<int size, int cols>
    static Eigen::Matrix<T, size, size>
    ukfCovariance(const Eigen::Matrix<T, size, cols> &x,
                  const Eigen::Matrix<T, size, 1> &mean,
                  const int n_aug = size + v_size) {
        const T lambda = defaultLambda(n_aug);
        const T mean_weight = lambda / (lambda + n_aug);
        const T other_weight = 1 / (2 * (lambda + n_aug));
    ...
    predict(const T &t) {
        ...
        const Eigen::Matrix<T, v_size, cols> noises = aug_sigma_points.block(x_size, 0, v_size, cols);
        if (verbose)
            cout << "aug_sigma_points" << endl << aug_sigma_points << endl;
        ...
        if (verbose)
            cout << "predicted_states" << endl << predicted_states << endl;
    ...
    void
    predictAndUpdate(const Event<T, y_size> &event) {
        const T &t = event.time;
        const Eigen::Matrix<T, y_size, 1> y = event.data;
        Measurement<T, x_size, y_size> *measurement_model = getMeasurementModel(event.type);
        // TODO Remove this after submitting for project. The get function should have an assertion.
        ...
        if (!initialized) {
            ...
            if (verbose)
                cout << "predicted_measurements" << endl << predicted_measurements << endl;
