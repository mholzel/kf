 ...
 * Q = G * Pv * G^T
 */
template<typename T = double, int x_size = Eigen::Dynamic, int v_size = Eigen::Dynamic>
class Dynamics {
...
public:
    ...
    Dynamics(const Eigen::Matrix<T, v_size, v_size> &Pv)
            : Pv(Pv) {
    }
    ...
    /** predict the state x(t+dt) given the previous state x(t) and nonzero process noise */
    ...
    virtual Eigen::Matrix<T, x_size, v_size>
    G(const T &t,
      const T &dt,
      const Eigen::Matrix<T, x_size, 1> &x) = 0;
