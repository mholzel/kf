...
public:
    ...
    error(const T &t,
         ...
         * However, the second entry of that vector is the bearing angle (in radians).
         * We need to be careful because we have no guarantee that the angle y(1)
         * is in the same range as our estimate yhat(1).
         * For instance, if y(1)= 0 and yhat(1) = 2 * pi + 0.1, the error
         * should not be 2 * pi + 0.1, but rather 0.1.
         * To find the correct angle, we use the dot product and then simply correct the sign
         * by projecting onto the vector normal to (px,py) */

        /* First, set the second component of yhat to 0 so that we can calculate the normal errors */
        yhat(1) = 0;
        ...
        /* When using low precision variables (like T = float), I have seen cosines slightly outside of [-1,1],
         * yielding acos(cos) = Nan. So before we compute the acos, we make sure that cos is strictly in this range. */
        if (cosine > 1)
            cosine = 1;
        else if (cosine < -1)
            cosine = -1;
