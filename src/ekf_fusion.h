...
private:
    ...
    std::unordered_map<int, MeasurementWithJacobian<T, x_size, y_size> *> &measurement_models;

    /** A handy function for retrieving the filter of the specified type and throwing an error if it doesn't exist. */
    MeasurementWithJacobian<T, x_size, y_size> *getMeasurementModel(const EventType &type) {
...
public:
    ...
    Eigen::Matrix<T, x_size, x_size>
    predictStateCovariance(const T &t) {
        T dt = t - this->t;
    ...
    /** update the internal state and state covariance predictions given the measurement y at the specified time.
     * Note that you will almost always want to call `predict` before this function so that you can be sure
     * that the internal state representation corresponds to the specified time. */
    void
    update(const Event<T, y_size> &event) {
        MeasurementWithJacobian<T, x_size, y_size> *measurement_model = getMeasurementModel(event.type);
        Eigen::Matrix<T, y_size, 1> e = measurement_model->error(event.time, x, event.data);
        ...
        Eigen::Matrix<T, y_size, y_size> S = C_P * Ct + measurement_model->Pw(event.time);
    ...
    /** update the internal state and state covariance predictions given the measurement y at the specified time */
    void
    predictAndUpdate(Event<T, y_size> &event) {
        if (!initialized) {
            initialized = true;
            this->t = event.time;
