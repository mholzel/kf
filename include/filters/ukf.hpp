...
class UKF {
 ...
 public:
  ...
  UKF(T &t, Eigen::Matrix<T, x_size, 1> &x, Eigen::Matrix<T, x_size, x_size> &P,
      Dynamics<T, x_size, v_size> &dynamics,
      Measurement<T, x_size, y_size> &measurement_model)
      : t(t),
        x(x),
  ...
  template <int cols>
  static Eigen::Matrix<T, x_size, 1> ukfMean(
      const Eigen::Matrix<T, x_size, cols> &predicted_states,
  ...
  template <int cols>
  static Eigen::Matrix<T, x_size, x_size> ukfCovariance(
      const Eigen::Matrix<T, x_size, cols> &predicted_states,
    ...
    const T mean_weight = lambda / (lambda + n_aug);
    const T other_weight = 1 / (2 * (lambda + n_aug));
    const Eigen::Matrix<T, x_size, cols> X_x =
        predicted_states.colwise() - mean;
  ...
  Eigen::Matrix<T, x_size, 2 * (x_size + v_size) + 1> predict(const T &t) {
    ...
    const Eigen::Matrix<T, x_size, cols> noises =
  ...
  void predictAndUpdate(const T &t, const Eigen::Matrix<T, y_size, 1> &y) {
    if (!initialized) {
      ...
      Eigen::Matrix<T, y_size, cols> ybar =
          ukfMean(predicted_measurements, n_aug);
      Eigen::Matrix<T, y_size, cols> S =
          measurement_model.Pw(t) +
          ukfCovariance(predicted_measurements, ybar, n_aug);
